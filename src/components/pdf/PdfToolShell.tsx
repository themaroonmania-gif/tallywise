  ...
  return (
    <div
      ...
      className={`cursor-pointer rounded-[1.75rem] border-2 border-dashed p-8 text-center transition-all md:p-10 ${
        dragging
          ? 'border-[#1463ff] bg-[#e8f0ff]'
          : 'border-[#d6e0ec] bg-[#f6f9fd] hover:border-[#1463ff]/55 hover:bg-white'
      }`}
    >
      <input
        ref={inputRef}
      ...
      />
      <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#10243e] text-[#76a7ff] shadow-xl shadow-slate-900/10">
        <UploadCloud className="h-7 w-7" />
      </div>
      <p className="font-display text-2xl font-black tracking-tight text-[#10243e]">{label ?? 'Select files'}</p>
      <p className="mt-2 text-sm font-semibold text-[#52677f]">{hint ?? 'or drag and drop them here'}</p>
    </div>
  );
}
...
export function PrimaryButton({ onClick, disabled, loading, children }: PrimaryButtonProps) {
  return (
    <button
      ...
      className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#1463ff] px-6 py-4 text-sm font-black uppercase tracking-[0.12em] text-white shadow-xl shadow-[#1463ff]/20 transition-all hover:-translate-y-0.5 hover:bg-[#0f52d4] disabled:cursor-not-allowed disabled:bg-[#9fb0c3] disabled:shadow-none"
    >
...
export function ResultPanel({ filename, size, onDownload, onReset, note }: ResultPanelProps) {
  return (
    <div className="rounded-[1.75rem] border border-[#0f766e]/20 bg-[#ecf8f4] p-6 text-center">
      ...
      <p className="font-display text-2xl font-black text-[#10243e]">Your file is ready</p>
      <p className="mt-1 truncate text-xs font-semibold text-[#52677f]">
        {filename}
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-center">
        ...
        <button
          onClick={onReset}
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#d6e0ec] bg-white px-6 py-3 text-sm font-black text-[#334a63] transition-all hover:bg-[#f6f9fd]"
        >
          Start over
        </button>
      </div>
