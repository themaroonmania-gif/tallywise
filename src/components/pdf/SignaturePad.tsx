...
function UploadPad({ onChange }: { onChange: (dataUrl: string | null) => void }) {
  ...
  const loadSignature = (file: File | undefined) => {
    if (!file) return;
  ...
  return (
    <div>
      ...
      <label className="flex min-h-32 cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-white p-4 text-center text-sm font-bold text-slate-500 transition-colors hover:border-[#1463ff]/50 hover:text-[#0f52d4]">
        {preview ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={preview} alt="Uploaded signature preview" className="max-h-28 max-w-full object-contain" />
        ) : (
          'Choose a PNG or JPG signature'
        )}
        <input
          type="file"
          accept="image/png,image/jpeg"
          className="sr-only"
          onChange={(event) => loadSignature(event.target.files?.[0])}
        />
      </label>
    </div>
  );
