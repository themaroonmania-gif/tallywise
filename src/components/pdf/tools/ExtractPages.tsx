...
export function ExtractPages() {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (!indices.length) { setError('Enter valid pages to extract, e.g. "1-3, 5".'); setLoading(false); return; }
      ...
      setResult({ blob, name: `${baseName(file.name)}-extracted.pdf`, size: blob.size });
    } catch (e) { console.error(e); setError('Something went wrong while extracting pages.'); }
    ...
    <PdfToolShell>
      {!file ? (
        ...
        <>
          ...
          <div>
            <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Pages to keep</label>
          ...
          <PrimaryButton onClick={run} loading={loading}>{loading ? 'Extracting…' : 'Extract pages'}</PrimaryButton>
        </>
      )}
